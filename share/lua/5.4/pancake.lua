...
--
-- Pancake aims to work with Pandoc ≥ v2.0.4.
-- However, it has only been tested with v2.9–v2.18, and only on Linux and macOS.
--
--
-- Tip:
-- ====
...
--
-- * Scalars are updated unless the function returns `nil`.
...
do
    ...
    -- Get the type of the items in a list.
    --
    ...
    elseif PANDOC_VERSION < {2, 17} then
        ...
        function el_type (el, ...)
            ...
            if t == 'userdata' or t == 'table' then
                ...
                -- pandoc.utils.type doesn't detect all lists.
...
do
    ...
    -- (case is ignored).
    --
    -- If an option is declared to be an 'list', its value is required to be
    -- of the Pandoc type `pandoc.List` or convertible to that type. However,
    -- if a scalar value is encountered where a list is expected, the value
    -- is wrapped in a single-item list automatically.
    --
    -- The items in a list must all be of the same type, which is declared
    -- by appending '<*T*>' to 'list', where *T* is either the name of a
    -- scalar type or another list declaration; *T* defaults to 'string'.
    --
